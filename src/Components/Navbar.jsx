...
export function Navbar({loc}){
    return(
        <div className="flex ">
        <Link to="/" className={`${loc?"view":"hidden"} backdrop-blur-xl backdrop-brightness-75 text-cp2bg2 hover:text-red-700 flex justify-center items-center`} ><ArrowCircleLeft size={32} weight="duotone" /></Link>
        <m.nav 
        initial={{
            y:-200
        }}
        ...
        className={`w-screen h-nav backdrop-brightness-75 pb-2 ${loc?"text-cp2bg2":"text-cp2bg1"}  flex tablet:flex-col smartphone:flex-col justify-between items-center sticky top-0`}>
            <a href="/"><h1 className="ubuntu transition-all text-5xl"> <span className="text-cp2accent pr-3" >{"<"}</span>MohdBilal<span className="text-cp2accent pl-5">{"/>"}</span> </h1></a>
